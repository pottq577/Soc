...
def load_wyscout_dataset(server_url='http://127.0.0.1:5000', dataset_name='England'):
    # 서버 엔드포인트에서 데이터 가져오기
    competitions = fetch_data_from_endpoint(f"{server_url}/competitions")
    teams = fetch_data_from_endpoint(f"{server_url}/teams")
    players = fetch_data_from_endpoint(f"{server_url}/players")
    matches = fetch_data_from_endpoint(f"{server_url}/matches/{dataset_name}")
    events = fetch_data_from_endpoint(f"{server_url}/events/{dataset_name}")

    if not all([competitions, teams, players, matches, events]):
        return None

    # 사전 생성
    competition_dict = {comp['wyId']: comp for comp in competitions}
    team_dict = {team['wyId']: team for team in teams}
    player_dict = {player['wyId']: player for player in players}
    match_dict = {match['wyId']: match for match in matches}
...
def refine_and_save_events(server_url='http://127.0.0.1:5000', dataset_name='England'):
    dataset = load_wyscout_dataset(server_url, dataset_name)
    if dataset is None:
        print("데이터 로드 실패.")
        return

    competition_dict, team_dict, player_dict, match_dict, match_event_dict = dataset

    # 정제된 이벤트를 위한 디렉토리 생성
    events_dir = f'data/refined_events/{dataset_name}'
    if not os.path.exists(events_dir):
        os.makedirs(events_dir)

    # 경기 및 이벤트 데이터 정제 및 저장
    match_df = refine_matches(team_dict, match_dict)
    match_df.to_csv(f'{events_dir}/matches.csv', encoding='utf-8-sig')

    for match_id, events in tqdm(match_event_dict.items(), desc=f"{dataset_name} 이벤트 처리 중"):
        refined_events = refine_match_events(team_dict, player_dict, match_dict, events)
        refined_events.to_pickle(f'{events_dir}/{match_id}.pkl')

# refine_matches 및 refine_match_events 함수는 변경 없이 그대로 사용
